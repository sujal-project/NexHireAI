...
#     for job in jobs:
#         job_skills = set([s.strip().lower() for s in job.skills.split(",") if job.skills])

#         score = len(user_skills & job_skills) / max(len(user_skills | job_skills), 1)
...
#         score = len(set(user_skills) & job_skills) / max(len(set(user_skills) | job_skills), 1)
